...
public class PackageCreatorFXMLController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        /* Start by adding packages and items to ComboBoxes and cities to 
        source and destination ComboBoxes. */
        
        FXMLDocumentController.writeLog("Pakettien ja esineiden luonti ikkuna avattu.");
    ...
    private void addItemAction(ActionEvent event) {
        /* Add item to package if there is space for it.
        Also updates main scene deliveryCombo. */
    ...
    private void addItemToPackage(int i, int Type, int Package) {
        /* Add given amount of given item to given package and calls databaseHandler. */
        
        dbh = getDbh();
        while (i-- > 0) {            
            dbh.addItemToDB(Type, Package);
        }
        FXMLDocumentController.writeLog("Esine(itä) lisätty pakettiin.");
        refreshPackageInfo();
    }

    @FXML
    private void createItemTypeAction(ActionEvent event) {
        /* Creates new item type and check if all given information are 
        correctly given. */
        ...
        try {
            ...
            FXMLDocumentController.writeLog("Uusi esine " + itemName + " luotu.");
...
int ID, String Name, double Size, double Weight, boolean Breakable, int Percent */
    private void createItemObject(ArrayList A) {
        /* Creates specific item object based on type selection button. */
...
int pID, int dID, double s, int fromID, int toID, String fromSP, String toSP */
    private void createPackageObject(int id, int did, double ps, int fromID, 
        ...
        /* Create package object and add it to package ComboBox. */
    ...
    private void createPackageAction(ActionEvent event) {
        /* Create package and check if class 1 delivery distance is over 150 km. */
    ...
    private void yesAction(ActionEvent event) {
        /* When creating package and confirmation pressed yes (Kyllä).
        Calls DBHandler to add packageDelivery and package to database. */
    ...
    private void noAction(ActionEvent event) {
        /* When creating package and confirmation pressed no (Ei). 
        Return to situation before confirmation asking. */
    ...
    private int checkBreakType(String t, int p) {
        /* Check if given breakType is new. If it is add it to database. */
        dbh = getDbh();
        int breakTypeID  = 0;
    ...
    private void refreshList(ListView L, ComboBox<String> C) {
        /* Refresh given ListView based on selection value of given ComboBox. */
        try {
            L.getItems().clear();
    ...
    private void refreshPackageInfo() {
        /* Update information in packageInfoLabel. 
        Example Empty space and items in package. */
        itemPackage P = packageCombo.valueProperty().getValue();
    ...
    private String parsePackageInfo(int ID) {
        /* Parses items in package string. Set in format "Number x itemName" */
        int i = 0;
        String temp = "";
    ...
    private void refreshItemInfoLabel(ActionEvent event) {
        /* Update itemInfoLabel based on selected item in item ComboBox. */
        String s = "Ei särkyvä";
    ...
    /* All radioActions diselect other radio buttons when any of them is selected.
    Change deliveryType integer to 1, 2 or 3 based on which one is selected.*/
    @FXML
    ...
    private void changeTypeAction(ActionEvent event) {
        /* Change item type when type changer button is pressed.
        Types are: Nnrmal, breaker, protective and special. */
    ...
    private String typeToEnglish(String s) {
        /* Change given type name to English. */
    ...
    private void cancelAction(ActionEvent event) {
        /* Close PackageCreator scene. */
        FXMLDocumentController.writeLog("Pakettien ja esineiden luonti ikkuna suljettu.");
        Stage packageCreator = (Stage) cancelButton.getScene().getWindow();
        packageCreator.close();
    }

    private void addCityToList() {
        /* Populate start and destination city ComboBoxes */
